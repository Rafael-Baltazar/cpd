...
void populate_cell(int line, int col, char type, struct world **matrix) {
	...
	if(type == 'w') {
		matrix[line][col].type = WOLF;
		matrix[line][col].starvation_period = w_starvation_p;
	   	matrix[line][col].breeding_period =	w_breeding_p;
	}
	else if(type=='s') {
		matrix[line][col].type = SQUIRREL;
		matrix[line][col].breeding_period = s_breeding_p;
	}
	else if(type=='i') {
		matrix[line][col].type = ICE;
	}
	else if(type=='t') {
		matrix[line][col].type = TREE;
	}
	else if(type=='$') {
		matrix[line][col].type = SQUIRRELnTREE;
		matrix[line][col].breeding_period = s_breeding_p;
	}
	else {
...
void scatter_matrix_from_file(char *file_name) {
	...
	//printf("PO: %d\n", cur_num_lines+ghost_lines_at_end(p));
	/*  */
	while((fscanf(file, "%d %d %c", &i, &j, &c)) != EOF) {	
		...
		while(i >= global_index + cur_num_lines + ghost_lines_at_end(p)) {
			...
			if(!p) {
				memcpy(worlds[0][0], buffer[0], total_lines * max_size * sizeof(struct world));
				memcpy(worlds[1][0], buffer[0], total_lines * max_size * sizeof(struct world));
			}
			else {
			...
			//printf("P%d: %d\n", p, global_index+cur_num_lines+ghost_lines_at_end(p));
		}
...
int main(int argc, char **argv) {
	if(argc >= N_ARGS) {
		...
		if(!id) {
			...
			printf("---P%d---\n", id);
			int i, j;
			int lim = ghost_lines_at_start(id) + get_num_lines(max_size, nprocs, id) + ghost_lines_at_end(id);
			for(i=0;i<lim;++i){
				for(j=0;j<max_size;++j) {
					printf("[%d][%d] %d",i ,j, worlds[1][i][j].type); 
				}
				printf("\n");
